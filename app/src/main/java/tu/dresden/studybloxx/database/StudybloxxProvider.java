		...
		switch (mURIMatcher.match(uri))
		{
			...
			case NOTE_ID:
			{
				...
				Cursor noteCursor = database.query(StudybloxxDBHelper.NOTE_TABLE_NAME, new String[] { StudybloxxDBHelper.NOTE_URL,
					StudybloxxDBHelper.NOTE_SYNCED }, StudybloxxDBHelper.NOTE_ID + "=?", new String[] { Long.toString(noteId) }, null, null, null);
				noteCursor.moveToFirst();
				String noteUrl = noteCursor.getString(0);
				boolean courseSynced = noteCursor.getInt(1) == 1;
				if (!courseSynced)
				{
					String noteContents = HttpFetch.get(Constants.STUDYBLOXX_HOST + noteUrl + "?username=" + Helper.getStoredUserName(getContext())
				...
				noteCursor = database.query(StudybloxxDBHelper.NOTE_TABLE_NAME, projection, StudybloxxDBHelper.NOTE_ID + "=? AND "
					+ StudybloxxDBHelper.NOTE_SYNCED + "=1", new String[] { Long.toString(noteId) }, null, null, null);
